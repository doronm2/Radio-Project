...
int close_connections(struct UDP_DATA *root_station,pthread_t new_station);
...
int download_song(char* song_name,ssize_t  song_len,int socket, struct sockaddr_in addr);
int send_message(int socket,SERVER_RESPONSE ans,int str_len,char* reply_str,struct sockaddr_in addr);
int initial_connection(struct TCP_DATA *my_data,socklen_t addr_len);
...
int main(int argc, char* argv[])
{
	...
	while(END_SERVER==0) {
		...
		retval = select(welcomeSocket+2, &rfds, NULL, NULL, &tv); // Watch stdin and tcp_sock to see when it has input.
		if(FD_ISSET(welcomeSocket, &rfds)){
			client.socket = accept(welcomeSocket, (struct sockaddr *)&serverStorage, &addr_size);
			...
			strcpy(client_ip,client.clientIP);
			client.ClientAddr.sin_family= AF_INET; /* Address family = Internet */
			client.ClientAddr.sin_port=htons((uint16_t )tcp_port);
			client.ClientAddr.sin_addr.s_addr=ipAddr.s_addr;
			if(DEBUG_MODE==1)
			...
			Client_List[client_ind].clientIP=(char*)calloc(INET_ADDRSTRLEN, sizeof(char));
			strcpy(Client_List[client_ind].clientIP,client.clientIP);
			Client_List[client_ind].client_thread=client.client_thread;
			Client_List[client_ind].client_index=client_ind; //each station starts on station 0
			Client_List[client_ind].active=1; //mark as active station
			Client_List[client_ind].station_num=0; //each station starts on station 0
	...
	if(close_connections(ROOT_ST,new_station)==0)
...
void* handle_station(void* data){
	...
	int str_len=strlen(inet_ntoa(my_data->ip_address));
	char Multicast_IP[str_len];
...
int close_connections(struct UDP_DATA *root_station,pthread_t new_station){
	...
	for(index=0;index<100;index++){
		if(Client_List[index].active==1) {
			if(Client_List[index].clientIP!=NULL)
...
void* handle_client(void* data){
	...
	int optval=1;
	socklen_t optlen =sizeof(optval),addr_len;
	char buffer[Buffer_size+1],*song_name,*ending="mp3",*cmp_str;
	uint8_t commandType = 0,song_name_len, buf32[4];
	uint16_t client_data;
	uint32_t song_size;
	addr_len=sizeof(my_data->ClientAddr);
	Hello_flag=initial_connection(my_data,addr_len);
	if(Hello_flag==-1)
		msg_err=1;

	setsockopt(my_data->socket,SOL_SOCKET,SO_KEEPALIVE,&optval,optlen);
	while(END_SERVER==0 && msg_err==0){
		msg_len= (int) recvfrom(my_data->socket, buffer, Buffer_size , 0,(struct sockaddr *)&my_data->ClientAddr,&addr_len);
		if(msg_len>0){
			commandType=(uint8_t )buffer[0];
			switch (commandType) {
			case HELLO:
				//send an invalid command to the client
				send_message(my_data->socket, INVALID_MSG,33,"Multiple Hello Message received\0",my_data->ClientAddr);
			...
			case ASKSONG:
				...
				if(Hello_flag==1){
					...
					if((client_data>=0 && client_data<NUM_OF_STATIONS)){
						...
						send_message(my_data->socket,ANNOUNCE,strlen(song_name),song_name,my_data->ClientAddr);
						Client_List[my_data->client_index].station_num=client_data; //change station number
					}
					else {
						//send an invalid command to the client
						send_message(my_data->socket, INVALID_MSG,46,"Client asked for a station hat doesn't exist\0",my_data->ClientAddr);
						msg_err = 1;//there is a error with the message - close connection
					}
				}
				else {
					//send an invalid command to the client
					send_message(my_data->socket, INVALID_MSG,31,"Hello Message wasn't received\0",my_data->ClientAddr);
					msg_err = 1;//there is a error with the message - close connection
				}
			...
			case UPSONG:
				...
				if(!(song_size>=MIN_SONG_SIZE && song_size<=MAX_SONG_SIZE)) {
					//send an invalid command to the client
					send_message(my_data->socket, INVALID_MSG,28,"Song size isn't perrmitted\0",my_data->ClientAddr);
					msg_err = 1;//there is a error with the message - close connection
				}
				else {
					...
					if(cmp_str==NULL){
						//send an invalid command to the client
						send_message(my_data->socket, INVALID_MSG,22,"Song name isn't .mp3\0",my_data->ClientAddr);
						msg_err = 1;//there is a error with the message - close connection
					}
					else{
						...
						if (check_song == 0) {
							if(DOWNLOAD_SONG==0){
								send_message(my_data->socket, PERMIT,1,NULL,my_data->ClientAddr); //permit the client to upload the song
								//upload the song to the server
								check_upload = download_song(song_name, song_size, my_data->socket,my_data->ClientAddr);
								if (check_upload == -1) {
									send_message(my_data->socket, INVALID_MSG, 28, "Problem Uploading song\0",my_data->ClientAddr);
									msg_err = 1;
								}
								else
								...
								send_message(my_data->socket, PERMIT, 0, NULL,my_data->ClientAddr);
						}
						else{
							send_message(my_data->socket, PERMIT, 0, NULL,my_data->ClientAddr);
						}
			...
			default: //if any other CommandType except 0,1,2 is receive, send a INVALID message
				//send an invalid command to the client
				send_message(my_data->socket, INVALID_MSG,28,"Wrong CommandType received\0",my_data->ClientAddr);
				msg_err = 1;//there is a error with the message - close connection
				break;
			}
		}
...
int initial_connection(struct TCP_DATA *my_data,socklen_t addr_len){
	...
	if(msg_len>0){
		...
		if(commandType==0){
			client_data=(uint16_t )((buffer[1]<<8)+buffer[2]);
			if (client_data==0) {
				send_message(my_data->socket, WELCOME,0,NULL,my_data->ClientAddr); //send WELCOME_MESSAGE to the client
				Hello_flag=1;
			}
			else {
				//send an invalid command to the client
				send_message(my_data->socket, INVALID_MSG,22,"wrong reserved Value\0",my_data->ClientAddr);
		...
		//send an invalid command to the client
		send_message(my_data->socket, INVALID_MSG,23,"Hello Message timeout\0",my_data->ClientAddr);
		Hello_flag = -1;//there is a error with the message - close connection
	}
	return Hello_flag;
}
			...
			if(my_data->num_of_station==NUM_OF_STATIONS){
				...
				for(i=0;i<100;i++) //ANNOUNCE to all clients on new station!
					if(Client_List[i].active==1)
						send_message(Client_List[i].socket,NEW_STATION,NUM_OF_STATIONS,NULL,Client_List[i].ClientAddr);
				DOWNLOAD_SONG=0;
			}
		}
	}
...
int send_message(int socket,SERVER_RESPONSE ans,int str_len,char* reply_str,struct sockaddr_in addr){
	...
	//Send the message
	if(sendto(socket,buffer,msg_len*sizeof(char),0,(struct sockaddr*)&addr,(socklen_t)sizeof(addr)) == -1) {
...
int download_song(char* song_name,ssize_t song_len,int socket, struct sockaddr_in addr){
	...
	while(checkUp>0) {
		...
		if(FD_ISSET(socket,&readfd)){
			if((checkUp=recvfrom(socket,buffer, sizeof(buffer),0,(struct sockaddr *)&addr,&addr_len)>0)){
				count_size+=checkUp;
				packet_ctr++;
