...
int main(int argc, char* argv[]) // Input: SERVER's IP_address/NAME, SERVER's TCP WELCOME port.
{
	...
	udpData = setTCP_sock(SERVER_TCP_IP,SERVER_TCP_port);
...
UDP_sock_data handShake()
{
	...
	inet_aton(welc_msg.mGroup_IP, &reference_mCast_addr);
	current_mCast_addr = reference_mCast_addr;

	welc_msg.mGroup_port = ntohs(((uint16_t*)(welc_buff+7))[0]);
